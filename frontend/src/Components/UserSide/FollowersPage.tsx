...
const FollowersPage = () => {
  ...
  useEffect(() => {
    const updateUsers = async () => {
      try {
        const { data } = await ClientNew.get(`${API_CHAT_URL}/findFollowers`, {
          headers: {
            "Content-type": CONTENT_TYPE_JSON,
          },
        });
        if (data.message === "Get all followers") {
          console.log(data.followers,'22222222222222222222222222222222222222');
          
          setgetAlluser(data.followers);
        } else {
  ...
  const followUser = async (userId: string, LoguserId: string) => {
    try {
      const { data } = await ClientNew.post(`${API_CHAT_URL}/followuser`,{ userId, LoguserId },
        {
          headers: {
            "Content-type": CONTENT_TYPE_JSON,
          },
