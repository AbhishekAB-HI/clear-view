...
router.get("/:chatId", AuthenticationMiddleware, async (req, res) =>
  MessageController.allChats(req, res)
);
router.patch("/blockuser", AuthenticationMiddleware, async (req, res) => MessageController.blockUserNow(req, res));
router.get("/getStatus",async (req, res) =>  MessageController.blockUserStatus(req, res));
