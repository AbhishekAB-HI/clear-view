...
class ChatController {
  ...
  async getAllUser(req: Request, res: Response) {
    try {
      ...
      console.log(userId, "getttttttttttttttttttttttttt");

      const data = await UserSchemadata.find({
        _id: { $ne: userId },
      });
      res.json(data);
    } catch (error) {
  ...
  async accessChat(req: Request, res: Response) {
    ...
    // console.log(fullChat, "get her first controllerssssssssssssss");
    ...
    //   const fullChat = await ChatSchemamodel.findOne(
    //     createChat._id
    //   ).populate("users", "-password");
    //   res
    //     .status(200)
  ...
  async fetchChat(req: Request, res: Response) {
    try {
      ...
      // Proceed with the database query if user is authenticated
